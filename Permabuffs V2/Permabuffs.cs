			...
			if (!permas.ContainsKey(id))
			{
				if (loadDBInfo(id))
				{
                    if (permas[id].bufflist.Count > 0)
                        TShock.Players[args.Who].SendMessage("Your list of rewarded buffs: " + string.Join(", ", permas[id].bufflist.Select(p => TShock.Utils.GetBuffName(p))) + "", TShock.Players[args.Who].Group.R, TShock.Players[args.Who].Group.G, TShock.Players[args.Who].Group.B);
				}
				else
